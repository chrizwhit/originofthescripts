...
.PARAMETER Licenses
    Array, defines the licenses used and to activate, specific set of licenses supported. "K1","K2","E1","E3","A1S","A2S","A3S","A1F","A2F","A3F","g4S"
...
.NOTES
    Be sure to check the licenses in use and the attribute you are synching against before running the script. Use Msol-AccountSKU to see available licenses in the tennent.
#>
param(
...
[ValidateSet("K1","K2","E1","E3","A1S","A2S","A3S","A1F","A2F","A3F","g4S","g4F")]
...
$SupportedSKUs = @{
                    ...
                    "g4S" = "STANDARDWOFFPACK_IW_STUDENT"
		                "g4F" = "OFFICESUBSCRIPTION_FACULTY"
                    }
...
Param([string]$SKU)
	...
	#If no switch has been used, process users and licenses from MSOnline
			    ...
			    $UsersToActivate = Get-MsolUser -All
				    if ($Userstoactivate)
				    {
...
if (!$licenses) {
    LogWrite "Error: No licenses specified, please specify a supported license`r`nInfo: Supported licenses are: K1,K2,E1,E3,A1S,A2S,A3S,A1F,A2F,A3F,g4S,g4F!"
}
else
{
